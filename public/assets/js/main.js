/*---------------------------------------------
Template name:  Hammrly
Item Description: Hammrly – Find Your Perfect Property
Version:        1.0
Author:         Hammrly
    ...
    $window.on('load', function () {
        ...
        $document.on('click', function(e) {
            if ($(window).width() <= 1199) {
                if (!$(e.target).closest('.dashboard-sidebar, #sidebarToggleTop').length) {
                    $('.dashboard-sidebar').removeClass('sidebar-is-active');
                    $('body').removeClass('sidebar-open');
                }
            }
        ...
        $document.on('click','.menu-toggle', function (e) {
            ...
            e.stopPropagation();
            var $toggle = $(this);
            var $header = $toggle.closest('.header-area');
        ...
        if ($(cardCarousel).length) {
            $(cardCarousel).each(function() {
                var $carousel = $(this);
                var cardCarouselItemCount = $carousel.children().length;
                var cardCarouselMaxItems = 3;
                var cardCarouselDesktopItems = Math.max(1, Math.min(cardCarouselMaxItems, cardCarouselItemCount));
                // Owl clones items when loop/center are enabled. That’s desirable for long lists,
                // but for short lists (<= visible items) it looks like duplicates.
                var cardCarouselShouldLoop = cardCarouselItemCount > cardCarouselMaxItems;

                // Prevent duplicate/partial init if script runs again.
                ...
                $carousel.owlCarousel({
                    loop: cardCarouselShouldLoop,
                    items: cardCarouselDesktopItems,
                    nav: true,
                    dots: true,
                    smartSpeed: 700,
                    autoplay: true,
                    ...
                    // Keep layout consistent with grid cards (no centered slide)
                    center: false,
                    margin: 30,
                    rewind: !cardCarouselShouldLoop,
